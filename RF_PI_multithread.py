...
def run_ensemble(i, X_feature, y, acqui_fn, top_indices, n_initial, N, seed):
    ...
    indices, top_indices_cnt = pool_learning(
        X=X_feature,
        y=y,
        acqui_fn=lambda model, best_f: acqui_fn(model=model, best_f=best_f),
...
with ProcessPoolExecutor(max_workers=10) as executor:
    futures = [
        executor.submit(run_ensemble, i, X_feature, y, LogProbabilityOfImprovement, top_indices, n_initial, N, seed_list[i])
...
print(saved)
np.save('results_P3HT_RF_PI_Init5.npy', saved)